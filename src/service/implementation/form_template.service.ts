...
export class FormTemplateService extends BaseService<FormTemplate> {
  ...
  async getFormTemplateByIdForPublic(
    ...
    // Parse Sections if it's a string (MySQL returns JSON as string)
    if (typeof data.Sections === 'string') {
      data.Sections = JSON.parse(data.Sections);
    }

    // Parse nested Fields and Options in each section if needed
    if (data.Sections) {
      data.Sections = data.Sections.map((section: FormSectionResult) => {
        if (typeof section.Fields === 'string') {
          section.Fields = JSON.parse(section.Fields);
        }
        // Parse Options in each field
        if (section.Fields) {
          section.Fields = section.Fields.map((field: FormFieldResult) => {
            if (typeof field.Options === 'string') {
              field.Options = JSON.parse(field.Options);
            }
            // Ensure Options is an array (null becomes empty array)
