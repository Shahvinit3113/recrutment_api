...
export class FormTemplateService extends BaseService<FormTemplate> {
  ...
  override async getByIdAsync(
    ...
    const result = await this.getFormTemplateByIdForPublic(id, this._callerService.tenantId);
    if (!result.Entity) {
      return Result.toEntityResult<FormTemplate>(null!);
    }
    return result as Result<FormTemplate>;
  }
